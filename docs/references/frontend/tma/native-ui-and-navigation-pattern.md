...
- generated shadcn/ui buttons, TMA wrappers, and fixed light-only visual theme
- haptics
...
- Use in-page shadcn/ui `Button` components for CTAs. `components/ui` contains only unmodified output generated by pinned `shadcn@4.18.0` from preset `b6GzOWK7U` (Sera, olive/yellow) with Base UI.
- `TmaHapticButton` is a project-owned wrapper outside `components/ui`; it composes the pristine Button and triggers Telegram `impact('light')` once for an enabled activation.
- Generated Button files must not contain TMA props, defaults, haptics, classes, or behavior. Haptic behavior belongs in `TmaHapticButton` or at the consumer/screen level.
...
- Do not apply a dark root class or dark token override. Do not alter generated dark branches.
- Prevent first-paint flash with the app's fixed light base colors before hydration.
- Restore Geist Mono from the locked preset as the global app typography; do not override it with a system body stack.
...
- `NativePicker`, `DatePicker`, and `QueryState` are project-owned components outside `components/ui`; preserve their behavior and compose generated shadcn primitives only for presentation. `QueryState` supports `plain`/`card` variants and an `isEmpty` predicate.
...
- In-page CTA text, disabled state, and loading state match current step; any haptic wrapper is outside generated `ui`.
