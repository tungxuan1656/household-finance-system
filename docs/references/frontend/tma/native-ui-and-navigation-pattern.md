...
- Use in-page shadcn/ui `Button` components for CTAs. `components/ui` contains only unmodified output generated by pinned `shadcn@4.18.0` from preset `b6G3fhkA4` with Base UI.
...
- `NativePicker`, `DatePicker`, and `DataState` are project-owned components outside `components/ui`; preserve their behavior and compose generated shadcn primitives only for presentation.
