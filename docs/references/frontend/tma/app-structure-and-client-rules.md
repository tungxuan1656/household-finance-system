...
- TMA uses shadcn/ui with Base UI, generated by pinned `shadcn@4.18.0` from exact preset `b6G3fhkA4` (Lyra, yellow/neutral, Geist Mono, Lucide).
...
- `TmaHapticButton` lives outside `components/ui` and composes the pristine generated Button. `NativePicker`, `DatePicker`, and `DataState` also live outside `components/ui` and retain their behavior.
