...
- [x] (2026-04-22, owner: Codex) Added failing logout integration coverage and confirmed the current worker returned 404 on `POST /api/v1/auth/logout`.
- [x] (2026-04-22, owner: Codex) Implemented logout route, handler, and contract wiring while keeping exchange/refresh contracts stable.
- [x] (2026-04-22, owner: Codex) Added the unauthorized logout regression and verified session revocation blocks both protected access and refresh replay.
- [x] (2026-04-22, owner: Codex) Passed worker `lint`, `typecheck`, and `test`, then passed full `./init.sh`.
- [x] (2026-04-22, owner: Codex) Updated harness records and prepared the plan for completion archival.
...
- Outcome: Backend auth session exchange is complete. `POST /api/v1/auth/logout` now revokes the current session, returns `{"revoked":true}`, rejects missing bearer tokens, and causes both the old access token and refresh token replay to fail after logout.
- Gaps: No remaining known gaps for feat-008; frontend session storage and router orchestration remain in `feat-009`.
- Lessons: Reusing the existing session revocation helper kept the change small, and the new logout flow fit the current auth middleware/session model without widening the repository surface.
