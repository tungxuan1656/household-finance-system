...
- Use shadcn/ui with Base UI as the TMA component system, generated by pinned `shadcn@4.18.0` from exact preset `b6G3fhkA4`.
...
- Keep product and platform behavior outside generated `ui`: `TmaHapticButton` composes the generated Button, while `NativePicker`, `DatePicker`, and `DataState` are project-owned components outside `components/ui` and retain their existing behavior.
