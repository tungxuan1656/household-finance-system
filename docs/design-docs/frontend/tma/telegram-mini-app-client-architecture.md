...
- TMA adopts `shadcn/ui` with Base UI using preset `b6GzOWK7U` (Sera, olive/yellow, Geist Mono, Lucide).
- TMA UI is fixed light-only and does not sync its visual theme to Telegram.
- The first TMA shell should use a three-position tab rail: `Home`, a centered add-expense action, and `Statistics`; `Settings` and `Expenses` are secondary pages.
...
- Every route uses in-page shadcn `Button` CTAs. When a CTA needs the standard light impact, `TmaHapticButton` outside generated `ui` composes the pristine Button; the generated Button has no Telegram behavior.
...
- Use shadcn/ui with Base UI as the TMA component system, generated by pinned `shadcn@4.18.0` from exact preset `b6GzOWK7U` (Sera, olive/yellow).
- Keep every shadcn file in `apps/tma/src/components/ui` as unmodified preset output. Do not add TMA props, defaults, haptics, classes, or behavior to generated files.
- Keep product and platform behavior outside generated `ui`: `TmaHapticButton` composes the generated Button, while `NativePicker`, `DatePicker`, and `QueryState` are project-owned components outside `components/ui` and retain their existing behavior.
- Keep layout and finance semantics at the consumer or screen level, not in generated `ui`.
- Keep the UI fixed light-only. Do not sync visual theme colors to Telegram theme vars, apply a dark root class, or override dark tokens. Leave generated dark branches untouched.
- Restore the preset Geist Mono as the global app typography. Do not replace it with a system body font stack.
...
For the component boundary and client rules, see [app structure and client rules](../../../references/frontend/tma/app-structure-and-client-rules.md). For native controls and navigation, see [native UI and navigation pattern](../../../references/frontend/tma/native-ui-and-navigation-pattern.md).
...
- Do not use Telegram `BottomButton` or `MainButton` on any route. Use in-page generated shadcn `Button` CTAs, composed by `TmaHapticButton` when haptic feedback is required.
