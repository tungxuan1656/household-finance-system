...
import { createInvitation } from '@/features/invitations/api/invitation'
import type {
  InvitationRoleDTO,
  InvitationTtlHours,
...
export const HouseholdInviteDialog = ({
  ...
  const handleGenerateInviteLink = async () => {
    try {
      ...
      setInviteLink(`${window.location.origin}${createdInvitation.invitePath}`)
  ...
  return (
    <Dialog
      ...
      onOpenChange={(open) => {
        onOpenChange?.(open)
        if (isOpen === undefined) setIsDialogOpen(open)
