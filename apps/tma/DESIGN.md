...
- All shadcn files under `src/components/ui` remain unmodified output generated by pinned `shadcn@4.18.0` from that exact preset. They contain no TMA props, defaults, haptics, classes, or behavior.
...
- preset light-neutral treatment
...
TMA uses **shadcn/ui with Base UI** using exact preset `b6G3fhkA4` (Lyra, yellow/neutral, Geist Mono, Lucide), generated by pinned `shadcn@4.18.0`. Tailwind CSS v4 (`@tailwindcss/vite`) remains the styling substrate. Legacy `--tma-*` UI tokens and their utility aliases are transitional only and are removed after all consumers migrate.
