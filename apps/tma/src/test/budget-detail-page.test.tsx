import { act } from 'react'
import { createRoot, type Root } from 'react-dom/client'
import { MemoryRouter, Route, Routes } from 'react-router-dom'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'

import { BudgetDetailPage } from '@/features/budgets/pages/budget-detail-page'

vi.mock('@/features/budgets/api', () => ({
  useBudgetDeleteMutation: vi.fn(),
  useBudgetDetailQuery: () => ({
    data: undefined,
    isError: false,
    isLoading: true,
    refetch: vi.fn(),
  }),
  useBudgetStatusQuery: () => ({
    data: undefined,
    isError: false,
    isLoading: true,
    refetch: vi.fn(),
  }),
  useDeleteBudgetMutation: () => ({
    isPending: false,
    mutateAsync: vi.fn(),
  }),
  useUpdateBudgetMutation: () => ({
    isPending: false,
    mutateAsync: vi.fn(),
  }),
}))

vi.mock('@/features/home/api', () => ({
  useHouseholdsQuery: () => ({
    data: { items: [] },
  }),
  useReferenceCategoriesQuery: () => ({
    data: undefined,
  }),
}))

vi.mock('@/lib/telegram/bottom-button', () => ({
  hideBottomButton: vi.fn(),
}))

vi.mock('@/lib/telegram/haptics', () => ({
  impact: vi.fn(),
  selection: vi.fn(),
}))

describe('BudgetDetailPage', () => {
  let host: HTMLDivElement
  let root: Root

  beforeEach(() => {
    ;(
      globalThis as typeof globalThis & {
        IS_REACT_ACT_ENVIRONMENT?: boolean
      }
    ).IS_REACT_ACT_ENVIRONMENT = true

    host = document.createElement('div')
    document.body.append(host)
    root = createRoot(host)
  })

  afterEach(async () => {
    await act(async () => {
      root.unmount()
    })

    host.remove()
  })

  it('renders the loading state without falling through to router not-found', async () => {
    await act(async () => {
      root.render(
        <MemoryRouter initialEntries={['/budgets/budget-1']}>
          <Routes>
            <Route element={<BudgetDetailPage />} path='/budgets/:id' />
            <Route element={<div>Trang bạn mở không tồn tại.</div>} path='*' />
          </Routes>
        </MemoryRouter>,
      )
    })

    expect(host.textContent).toContain('Đang tải ngân sách')
    expect(host.textContent).not.toContain('Trang bạn mở không tồn tại.')
  })
})
