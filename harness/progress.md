...
- 2026-07-06 — TMA personal income capture MVP (feat-125): shipped worker `incomes` table + authenticated `GET/POST /api/v1/incomes`, TMA `/incomes` + `/incomes/new`, Home shortcut entrypoint, and a one-page add-income form with Telegram BottomButton save. Follow-up fixes covered split worker income specs for push hygiene, corrected TMA amount rendering/save text, added cursor-based income pagination, aligned `spentByUserId`, added worker soft-delete list filtering, and polished local-date + optional note handling with single-source error haptics. Verified with focused worker/TMA income tests, `./init.sh build`, and final `./init.sh`; Telegram-on-device manual QA remains unrun.
