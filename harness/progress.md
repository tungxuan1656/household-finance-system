...
- Summary: Refined the TMA auth bootstrap (feat-080) so a valid cached session skips the exchange API call entirely on cold open. The exchange response already returns `accessTokenExpiresIn` + `refreshTokenExpiresIn`; the client now persists the full session blob (access token, refresh token, both expiry timestamps, user, telegramUserId) in Telegram SecureStorage and compares expiry locally. Cold-open decision tree: no cache or telegramUserId mismatch -> exchange (fresh); access valid (>60s buffer) -> 0 API restore; access expired + refresh valid -> refresh (1 API); both expired -> exchange (1 API); in-session 401 -> refresh -> fail -> exchange -> fatal. Critical cross-user guard: telegramUserId mismatch NEVER reuses the stored refresh token (prevents cross-user data leakage on shared devices). Telegram user_id is a cache-invalidation key only, not authenticated identity — the server still verifies JWT signature, refresh token ownership, and initData signature. Also fixed the SecureStorage capability detection (was unconditionally `false`, so persistence was always disabled). Removed unreachable post-exchange refresh fallback dead code discovered during implementation.
- Files changed: `apps/tma/src/lib/telegram/capabilities.ts` (real SecureStorage detect); `apps/tma/src/lib/storage/keys.ts` (key `session:refresh-token` -> `session:tma-auth`); `apps/tma/src/lib/storage/adapter.ts` (full rewrite: `StoredSession` blob, `getSession`/`setSession`/`clearSession`, JSON parse guard, memory fallback preserved); `apps/tma/src/lib/telegram/launch-params.ts` (added `readTelegramUserId`); `apps/tma/src/features/auth/store.ts` (added `telegramUserId` + `refreshTokenExpiresAt`, `restoreSession` method); `apps/tma/src/features/auth/bootstrap-deps.ts` (full rewrite: `restore` phase before `exchange`, cross-user guard, expiry-aware branching); `apps/tma/src/features/auth/refresh-interceptor.ts` (persist full blob after refresh); `apps/tma/src/app/app.tsx` (wired new deps); `apps/tma/src/test/storage-adapter.test.ts` (rewrite, 9 tests); `apps/tma/src/test/auth-bootstrap.test.ts` (rewrite, 15 tests); `harness/feature_index.json`, `harness/features/feat-080.json`, `harness/progress.md`.
- Verification: `./init.sh typecheck` OK; `./init.sh test` OK (76 tests across 17 files); `./init.sh lint` OK.
- Blockers: None. SECURITY OVERRIDE: this explicitly overrides the locked default "access token memory-only" (`docs/references/frontend/tma/state-and-storage-pattern.md:43` permits override on explicit security review). Access token is now persisted in SecureStorage (Telegram v9.0+ encrypted device storage). Tradeoff: server-side revocation is delayed until access token TTL expiry or a 401, not instant on cold open. Documented in `adapter.ts` JSDoc + feat-080 evidence.
- Next steps: Real Telegram WebView smoke test — cold open with valid token should show 0 `/auth/exchange` calls in Network tab; expired access + valid refresh should show 1 `/auth/refresh` call. Commit when user requests.
