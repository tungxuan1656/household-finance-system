...
- Files changed: docs/exec-plans/plans/2026-05-05-feat-029-analytics-comparisons-breakdowns.md, docs/exec-plans/index.md, harness/features/feat-029.json, harness/feature_index.json, harness/progress.md
- Blockers: open implementation decision on whether comparison/groups data loads eagerly with overview or lazily per section/tab; ungrouped-expense treatment must be recorded explicitly if surfaced during implementation.
- Next steps: confirm existing analytics repository helpers from feat-028, define exact comparison/groups DTOs, implement backend endpoints/tests first, then extend insights page/components and finish with `./init.sh`.

## 2026-05-04 — Completed feat-028: Analytics overview dashboard
- Who: Orchestrator
- Summary: Implemented the fullstack analytics overview dashboard for `feat-028`. Backend: added dedicated analytics query schema/types, `GET /api/v1/analytics/overview`, and repository-owned monthly aggregation queries for total spend, expense count, daily spend series, and top-5 categories while preserving existing visibility rules so private expenses stay out of household analytics. Frontend: replaced `/insights` placeholder with month-based analytics page orchestration, typed transport/hook layer, summary cards, daily spend bar chart, top-categories pie chart with ranked list, loading/empty/error states, and Vietnamese locale keys resolved through the global category catalog. Added focused worker and web tests, installed `recharts` for chart rendering, and closed verification with full `./init.sh`.
- Files changed: apps/worker/src/contracts/analytics-schemas.ts, apps/worker/src/contracts/analytics-types.ts, apps/worker/src/contracts/index.ts, apps/worker/src/db/repositories/expense-query-repository.ts, apps/worker/src/handlers/analytics/get-analytics-overview.ts, apps/worker/src/routes/analytics.ts, apps/worker/src/index.ts, apps/worker/test/unit/dto-analytics.spec.ts, apps/worker/test/integration/analytics-overview.spec.ts, apps/web/src/types/analytics.ts, apps/web/src/api/analytics.ts, apps/web/src/api/endpoints.ts, apps/web/src/hooks/api/use-analytics.ts, apps/web/src/views/app/insights-page.tsx, apps/web/src/views/app/insights-page.test.tsx, apps/web/src/app/(protected)/insights/page.tsx, apps/web/src/lib/i18n/locales/vi.json, apps/web/package.json, docs/exec-plans/index.md, harness/features/feat-028.json, harness/feature_index.json, harness/progress.md
- Verification: `pnpm typecheck` (apps/worker); `pnpm typecheck` (apps/web); `pnpm test -- --run apps/worker/test/unit/dto-analytics.spec.ts apps/worker/test/integration/analytics-overview.spec.ts` (apps/worker); `pnpm test -- --run src/views/app/insights-page.test.tsx` (apps/web); `./init.sh`
- Blockers: none
- Next steps: feat-029 can extend this surface with comparisons and breakdowns without changing feat-028 scope.
...
- Files changed: apps/worker/src/contracts/budget-types.ts, apps/worker/src/db/repositories/budget-repository.ts, apps/worker/src/handlers/budgets/get-budget-status.ts, apps/worker/src/routes/budgets.ts, apps/worker/test/integration/budgets-status.spec.ts, apps/web/src/types/budget.ts, apps/web/src/api/endpoints.ts, apps/web/src/api/budget.ts, apps/web/src/hooks/api/use-budgets.ts, apps/web/src/views/app/budgets-page.tsx, apps/web/src/views/app/budgets-page.test.tsx, apps/web/src/components/budget/budget-status-card.tsx, apps/web/src/components/budget/budget-status-card.test.tsx, apps/web/src/components/budget/budget-status-panel.tsx, apps/web/src/components/budget/budget-status-panel.test.tsx, apps/web/src/components/budget/index.ts, apps/web/src/lib/i18n/locales/vi.json, docs/exec-plans/plans/2026-05-04-feat-027-budget-tracking-threshold-status.md, harness/features/feat-027.json, harness/feature_index.json, harness/progress.md
- Verification: `pnpm --filter worker test -- budgets-status.spec.ts`; `pnpm --filter web test -- apps/web/src/components/budget/budget-status-panel.test.tsx apps/web/src/components/budget/budget-status-card.test.tsx apps/web/src/views/app/budgets-page.test.tsx`; `pnpm --filter web typecheck`; `./init.sh`
- Blockers: none
- Next steps: feat-028 analytics overview can build on the new budget status surface without changing feat-027 scope.

## 2026-05-04 — Created active ExecPlan for feat-027 budget tracking & threshold status
- Who: Orchestrator
- Summary: Created and registered the active ExecPlan for `feat-027` covering fullstack budget tracking and threshold status. Backend scope: add `GET /api/v1/budgets/:id/status`, planned-vs-actual aggregate queries for the budget month using household/visibility-filtered expense data, and fixed-threshold status calculation (`ok`, `warning` at `>= 80%`, `exceeded` at `>= 100%`) for total and per-category budgets. Frontend scope: extend budget transport/hooks, evolve the budgets page into a tracking view, show total summary + remaining amount + inline warning alert, and render per-category progress rows resolved from the global category catalog. Explicitly out of scope: push/email notifications, configurable thresholds, and spend timeline/analytics charts.
- Files changed: docs/exec-plans/plans/2026-05-04-feat-027-budget-tracking-threshold-status.md, docs/exec-plans/index.md, harness/features/feat-027.json, harness/feature_index.json, harness/progress.md
- Blockers: visibility-filter semantics for household aggregates and current-period defaulting need confirmation during implementation, but do not block plan creation.
- Next steps: confirm the budget-status contract and aggregate query shape, implement backend status endpoint/tests first, then add frontend tracking UI and full verification via `./init.sh`.

## 2026-05-04 — Completed feat-026: Budget setup & editing (fullstack)
- Who: Orchestrator
- Summary: Implemented the fullstack budget setup & editing feature (feat-026). Backend: migration 0006 adding `category_key` to `budget_limits` and simplifying `budgets` scope model; budget CRUD contracts/schemas with admin-only create/edit and per-category sub-budgets keyed by global category catalog (kind=expense); budget repository with atomic category limit updates; handlers for create, get, list-by-period, update; route registration; unit schema tests (36) and integration CRUD tests (20). Frontend: budget types, API functions, React Query hooks; budget form with period selector and per-category rows from global catalog; create/edit dialogs; budget card and list components; budget summary card; budgets page replacing placeholder; Vietnamese i18n labels. All verification passed: lint, type-check, tests (310 worker tests), web build.
- Files changed: apps/worker/migrations/0006_budget_category_key.sql, apps/worker/src/contracts/budget-schemas.ts, apps/worker/src/contracts/budget-types.ts, apps/worker/src/contracts/index.ts, apps/worker/src/db/repositories/budget-repository.ts, apps/worker/src/handlers/budgets/create-budget.ts, apps/worker/src/handlers/budgets/get-budget.ts, apps/worker/src/handlers/budgets/list-budgets.ts, apps/worker/src/handlers/budgets/update-budget.ts, apps/worker/src/lib/permissions/household-policy.ts, apps/worker/src/routes/budgets.ts, apps/worker/src/index.ts, apps/worker/test/unit/dto-budget.spec.ts, apps/worker/test/integration/budgets-crud.spec.ts, apps/web/src/types/budget.ts, apps/web/src/api/budget.ts, apps/web/src/api/endpoints.ts, apps/web/src/hooks/api/use-budgets.ts, apps/web/src/components/budget/fields/schema.ts, apps/web/src/components/budget/fields/period-field.tsx, apps/web/src/components/budget/fields/total-limit-field.tsx, apps/web/src/components/budget/fields/category-limit-row.tsx, apps/web/src/components/budget/fields/category-limits-section.tsx, apps/web/src/components/budget/fields/index.ts, apps/web/src/components/budget/budget-form.tsx, apps/web/src/components/budget/create-budget-dialog.tsx, apps/web/src/components/budget/edit-budget-dialog.tsx, apps/web/src/components/budget/budget-card.tsx, apps/web/src/components/budget/budget-list.tsx, apps/web/src/components/budget/budget-summary-card.tsx, apps/web/src/components/budget/index.ts, apps/web/src/views/app/budgets-page.tsx, apps/web/src/app/(protected)/budgets/page.tsx, apps/web/src/lib/i18n/locales/vi.json, apps/web/src/components/ui/select.tsx (shadcn)
- Blockers: none
- Next steps: feat-027 (spend tracking & threshold alerts) can build on the budget data model

## 2026-05-04 — Created active ExecPlan for feat-026 budget setup & editing
- Who: Orchestrator
- Summary: Created and registered the active ExecPlan for `feat-026` covering fullstack budget setup and editing. Backend scope: migration 0006 to add `category_key` to `budget_limits` and update constraints; budget CRUD contracts/schemas (POST/GET/PATCH /api/v1/budgets with admin-only create/edit, per-category sub-budgets keyed by global category catalog limited to kind=expense); budget repository with atomic category limit updates; handlers for create, get, list-by-period, update; route registration; unit and integration tests. Frontend scope: budget types, API functions, React Query hooks, budget form with period selector and per-category rows from global catalog, budget summary card, budgets page replacing placeholder, i18n labels, component tests. Key decisions: use `category_key` (global catalog) instead of `category_id` (legacy FK) for per-category limits; only `scope='household'` in budgets table; period format YYYY-MM; admin-only create/edit; audit logging for budget changes. Explicitly out of scope: spend tracking, threshold alerts (feat-027), analytics charts, recurring rules, budget deletion/archival.
- Files changed: docs/exec-plans/plans/2026-05-04-feat-026-budget-setup-editing.md, docs/exec-plans/index.md, harness/features/feat-026.json, harness/feature_index.json, harness/progress.md
- Blockers: none
- Next steps: execute Step 1 (migration 0006), then Steps 2-6 (backend contracts, repository, handlers, routes, tests), then Steps 7-13 (frontend types, API, hooks, components, views, i18n, tests), then Step 14 (full verification via ./init.sh).

## 2026-05-04 — Completed feat-021: Expense search, filters & pagination
- Who: Orchestrator
- Summary: Implemented the scoped fullstack querying enhancements for `feat-021` and then closed the post-review contract/pagination gaps. Worker: extended `GET /api/v1/expenses` with validated note-search, amount range, creator, category, payer, visibility, date, group, sort, limit, and sort-aware cursor filters; added filtered aggregation via `GET /api/v1/expenses/summary` using the same effective filter set as the list path; and added unit/integration coverage for query parsing, filtered list behavior, invalid filters, summary totals, and `amount_desc` pagination stability. Web: aligned typed list/summary contracts to backend (`query`, `totalSpendMinor`, `expenseCount`), added typed summary transport and query hook support, wired page-local query/visibility/category controls into the expenses page, added a compact feed summary card, forwarded shared filters into both summary and infinite feed queries, and added component/page regression tests for the corrected wiring. Full verification passed via `PATH="$HOME/.nvm/versions/node/v24.15.0/bin:$PATH" ./init.sh`.
- Files changed: apps/worker/src/contracts/expense-schemas.ts, apps/worker/src/db/repositories/expense-repository.ts, apps/worker/src/db/repositories/expense-query-repository.ts, apps/worker/src/handlers/expenses/list-expenses.ts, apps/worker/src/handlers/expenses/get-expense-summary.ts, apps/worker/src/routes/expenses.ts, apps/worker/test/unit/dto-expense-list.spec.ts, apps/worker/test/integration/expenses-list.spec.ts, apps/worker/test/integration/expenses-summary.spec.ts, apps/web/src/api/endpoints.ts, apps/web/src/api/expense.ts, apps/web/src/hooks/api/use-expense.ts, apps/web/src/types/expense.ts, apps/web/src/components/expense/expense-feed-list.tsx, apps/web/src/components/expense/expense-feed-list.test.tsx, apps/web/src/components/expense/expense-feed-summary.tsx, apps/web/src/components/expense/expense-feed-summary.test.tsx, apps/web/src/components/expense/index.ts, apps/web/src/views/app/expenses-page.tsx, apps/web/src/views/app/expenses-page.test.tsx, harness/features/feat-021.json, harness/feature_index.json, harness/progress.md, docs/exec-plans/index.md, docs/exec-plans/plans/2026-05-04-feat-021-expense-search-filters-pagination.md
- Blockers: none
- Next steps: feat-026 (Budget setup & editing) is the next pending feature.
