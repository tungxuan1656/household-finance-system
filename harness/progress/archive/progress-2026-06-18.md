...
## 2026-06-18 — TMA amount helper consolidation + currency display unify + edit-input bug fix

- Who: GLM-5.2 (orchestrator) + explorer + 2 fixer lanes (subagent-driven-development)
- Summary: Three related pieces of work in one session. (1) **Research**: confirmed `amountMinor` is currency-aware minor units (`amount * 10^fractionDigits` via worker `getMinorUnits`), NOT hardcoded ×100; VND has fractionDigits=0 so `amountMinor = amount` 1:1, USD/EUR=×100, KWD=×1000. Read-back is `/10^fractionDigits` (currency-aware), not always `/100`. Web is fully currency-aware (`apps/web/src/utils/currency/format.ts` + `dialog-amount-helper.ts`); TMA display is currency-aware (`formatCurrencyMinor`) but the edit/add surfaces had scattered hardcoded `*1000`/`/1000` VND thousand-shortcut logic. (2) **Bug fix**: TMA edit expense page showed an empty amount input on back-then-reenter (first visit from detail filled correctly). Root cause: stale draft in the global `useEditExpenseStore` Zustand store — the hydrate effect guarded on `!draft`, so on re-enter the store still held the previous draft and `setAmountInput` was skipped while the local `amountInput` state re-initialized to `''`. Fixed by adding a cleanup `useEffect` that calls `resetStore()` on unmount in `expense-edit.tsx`. (3) **Refactor (feat-108)**: consolidated the scattered VND thousand-shortcut logic into 5 shared pure exports in `apps/tma/src/lib/formatters.ts` (`MINOR_MULTIPLIER`, `minorFromRaw`, `rawFromMinor`, `minorFromInput`, `currencyDisplaySymbol`) mirroring web's `dialog-amount-helper.ts`, and rewired 4 consumer files. Unified currency display so TMA shows `₫` instead of raw `VND` text. The `.000` suffix label stays hardcoded as a VND-only visual hint (out of scope to make currency-aware). No behavior change for the current VND-only app.
- Files changed: `apps/tma/src/routes/expense-edit.tsx` (bug fix: unmount reset effect; refactor: `*1000`→`minorFromRaw`); `apps/tma/src/lib/formatters.ts` (+5 exports); `apps/tma/src/features/expenses/draft.ts` (`/1000`→`rawFromMinor`); `apps/tma/src/routes/add-expense-details.tsx` (`/1000`→`rawFromMinor`, `*1000`→`minorFromRaw`, `VND` literal→`currencyDisplaySymbol('VND')`); `apps/tma/src/routes/expense-edit-form.tsx` (`{currencyCode}`→`{currencyDisplaySymbol(currencyCode)}`); `harness/feature_index.json`, `harness/features/feat-108.json` (new), `harness/progress.md`.
- Verification: `./init.sh typecheck` OK; `pnpm --filter tma lint` 0 errors (13 pre-existing `no-console` warnings in unrelated files; 2 `padding-line-between-statements` warnings introduced by the helper were fixed inline and the re-run returned to the 13-problem baseline). Grep confirmed zero amount-related `*1000`/`/1000` remain in the 4 consumer files.
- Blockers: None.
- Next steps: Commit when user requests. Future multi-currency support would derive the `.000` suffix and `MINOR_MULTIPLIER` from `fractionDigits` + locale instead of hardcoding VND; the helper is now the single place to update.
